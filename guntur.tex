...
% elastic model
Elastic Model (EM) \cite{Kim2013} focuses to intellegently meet users' need. This approach is slightly similar with the SCC because EM also use the four smart concept to cloud service.
context-aware -< yes
security -> yes
multidevice support -s. Yes
ontology 
flexibility -> yes
...
\begin{table}[htb]
  ...
  \begin{tabular}{lcccc}
    ...
    Context-awareness & \cmark &  & \cmark & \cmark \\
    Multidevice support & \cmark &  & $-$ & \cmark \\
    Ontology & \cmark &  & \cmark &  $-$ \\
    Flexibility & \cmark &  &  $-$ & \cmark
  \end{tabular}
  \end{center}
\end{table}
  ...
  Intuitive user interface and offers an out of the box user experience. Prevent duplication of data and can manage computing resources efficiently.
